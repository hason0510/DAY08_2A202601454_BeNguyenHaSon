...
def retrieve(
    ...
    """
    # Step 1: Chạy semantic + lexical (lấy dư top_k*2 để merge/rerank có biên độ)
    dense_results = semantic_search(query, top_k=top_k * 2)
    sparse_results = lexical_search(query, top_k=top_k * 2)

    # Step 4 (quyết định fallback) DÙNG ĐIỂM COSINE GỐC của semantic_search,
    # KHÔNG PHẢI điểm RRF đã fuse — xem ghi chú "BẪY THƯỜNG GẶP" ở đầu file.
    ...
    if best_score < score_threshold:
        ...
        except Exception as e:  # PageIndex chưa cấu hình / lỗi mạng → bỏ qua fallback
    ...
    # Step 2: Merge dense + sparse bằng RRF (hybrid search)
